...
- [ ] `watch-control-endpoint` — wire `POST /control` on `@minsky/dashboard-web` so the pause / resume Shortcuts actually mutate state
  - **ID**: watch-control-endpoint
  - **Tags**: novel, follow-up, dashboard-web, watch-shortcuts
  - **Estimate**: 2–3h
  - **Hypothesis**: The pause / resume Shortcuts shipped by `watch-shortcuts` POST `{"paused": true|false}` to `:8080/control`, but the `/control` route is **not yet implemented** on the dashboard-web side — the pause sentinel needs to land in a place the supervisor reads. Adding a `POST /control` handler that writes a paused-flag sentinel (or invokes a `setPaused` Strategy) closes the loop in <1 scheduler iteration (per the parent `watch-shortcuts` task's verification cell).
  - **Details**: Add a `POST /control` route to `createServer` that validates `{ paused: boolean }` (rule #7 — upstream-malformed graceful-degrade) and either writes a sentinel file the supervisor honors OR invokes a `setPaused: (v: boolean) => void` Strategy injected the same way as `getValue` / `getPauseState` (rule #2 adapter shape). Add tests: 200 + body parse on valid input; 400 on malformed input (missing key, non-boolean); the Strategy is called exactly once per accepted request; the `getPauseState` Strategy reflects the latest `setPaused` call (round-trip consistency). Tighten the smoke test in `distribution/shortcuts/test/shortcuts-json.test.mjs` to assert pause/resume payload schema once the server handler exists.
  - **Files**: `novel/dashboard-web/src/server.ts`, `novel/dashboard-web/src/watch.ts` (or new `control.ts`), `novel/dashboard-web/test/server.test.ts`, `novel/dashboard-web/README.md` (sub-task 7 block)
  - **Verification**: `pnpm vitest run novel/dashboard-web/test/server.test.ts` exits 0 with at least 4 new `POST /control` tests; round-trip `POST /control {paused:true}` → `GET /watch.json` body has `paused: true`; chaos table in `novel/dashboard-web/README.md` carries a row for "Malformed control payload (missing key, wrong type)" → `graceful-degrade` 400.
  - **Measurement**: `pnpm vitest run novel/dashboard-web/test/server.test.ts --reporter=json | jq -e '.numPassedTests >= 21 and .numFailedTests == 0'` exits 0 (today: 17, +4 new tests).
  - **Pivot**: if the supervisor-side wiring (the *consumer* of the paused sentinel) is materially harder than the dashboard-web side (>1d), retire `/control` entirely and ship pause/resume as a launchctl-only operator action documented in `distribution/launchd/README.md`. The Watch surface stays read-only — the user-story 002 Acceptance is preserved by the iPhone Shortcut still working at the OS level.
  - **Acceptance**: `POST /control` round-trips through `/watch.json`; supervisor-side wiring filed as a separate task if scope grows.
  - **Anchor**: Beyer SRE 2016 Ch. 17 (operator escape hatch / kill switch); rule #2 (adapter seam — `setPaused` Strategy is the seam); rule #7 (chaos engineering — malformed payload is the upstream fault axis).
  - **Risk**: Anyone on the Tailscale net can pause the loop. Mitigation: Tailscale ACLs scope the surface; rule #7's let-it-crash design means a malicious pause flips back on resume.
