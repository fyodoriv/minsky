...
- [ ] `daemon-otel-pipe` — daemon's `emit` writes spans to stdout; OpenObserve receives nothing
  - **ID**: daemon-otel-pipe
  - **Tags**: novel, observability, runtime, surfaced-by-audit
  - **Estimate**: 3–4h
  - **Hypothesis**: The daemon's `emit` callback in `novel/tick-loop/bin/tick-loop.mjs` writes plain-text spans to `process.stdout` instead of forwarding them to the configured OTEL backend. OpenObserve is installed and serves `:5080/healthz` (post-#110), but the daemon emits zero spans there — every `tick-loop.iteration` event lives only in the operator's terminal/journalctl. The dashboard's `OpenObserveStrategy` (PR #87) reads PromQL from OpenObserve and gets `(stub)` for every metric because the publisher side is missing. Wiring `@minsky/observability`'s OTEL exporter (it already has `OtelObservability` with an `endpoint` opt) into the CLI's `emit` callback closes the publisher half of the loop. Hypothesis: with `MINSKY_OTEL_ENDPOINT=http://localhost:5080` set, a 1-iteration daemon run emits ≥1 span readable from OpenObserve via PromQL, AND the dashboard's `tokens-remaining` field flips from `(stub)` to a real value within one tick.
  - **Details**: 1) Add a `@minsky/observability` dep to `novel/tick-loop/package.json`. 2) In `bin/tick-loop.mjs`, construct an `OtelObservability` (the existing strategy from PR #87) when `MINSKY_OTEL_ENDPOINT` is set; fall back to the current stdout `emit` when unset (graceful-degrade per rule #7). 3) Replace the `emit` callback with the OTEL adapter's span-writer; preserve the stdout line so operators don't lose terminal visibility. 4) Add an integration test gated on `OPENOBSERVE_INTEGRATION=1` (matching the existing convention in `novel/adapters/observability/test/`) that runs 1 dry-run iteration with `MINSKY_OTEL_ENDPOINT=http://localhost:5080` set, then PromQL-queries `tick-loop_iteration_total` (or whatever name the OTEL adapter exports) and asserts ≥1 sample. 5) Update vision.md row 67 to note the publisher/reader join.
  - **Files**: `novel/tick-loop/bin/tick-loop.mjs`, `novel/tick-loop/package.json`, `novel/tick-loop/src/daemon.test.ts` (gated integration test), `novel/adapters/observability/src/otel.ts` (verify endpoint opt is exposed), `vision.md` row 67
  - **Verification**: gated integration test passes locally with OpenObserve running; with the daemon stopped, the OpenObserve dashboard shows the daemon's iteration spans.
  - **Measurement**: `pnpm vitest run novel/tick-loop --reporter=json | jq -e '.numPassedTests >= 39 and .numFailedTests == 0'` (today: 38; +1 gated integration test on top).
  - **Pivot**: if `OtelObservability`'s exporter shape doesn't match the daemon's `TickSpan` (different attribute names, batch semantics), pivot to a thin adapter in the daemon CLI that maps `TickSpan` → OTEL Span before forwarding. Don't touch `@minsky/observability` itself.
  - **Acceptance**: `MINSKY_OTEL_ENDPOINT=http://localhost:5080 node bin/tick-loop.mjs --max-iterations=1 --tick-interval-ms=0` (with `MINSKY_TICK_DRY_RUN=1`) results in OpenObserve receiving ≥1 span per iteration; dashboard's `OpenObserveStrategy` reads it back; the publisher/reader loop is closed.
  - **Anchor**: Kephart & Chess 2003 (MAPE-K — the *Monitor* surface must publish where the *Analyze* phase reads); Beyer SRE 2016 Ch. 6 (every emitted metric must be read; every read must have a publisher — today the publisher half is missing); rule #4 (the OTEL coverage lint covers `@otel` JSDoc on exports, but the actual span-publishing pipe is what gives the JSDoc its operational value).
  - **Risk**: OTLP transport may add latency to the tick loop. Mitigation: the OTEL adapter already does async batched export by design; the daemon's `emit` is fire-and-forget. If the adapter blocks for >100ms per span, file a follow-up to make it explicitly async.
  - **Surfaced-by**: 2026-05-04 standing-audit-gap-loop pass — the daemon's `bin/tick-loop.mjs` `emit` callback writes to `process.stdout` instead of OpenObserve; OpenObserve was installed via #110 but receives nothing; the dashboard's `OpenObserveStrategy` reads `(stub)` because the publisher side never wired up. Closes the publisher half of the publish-then-read MAPE-K loop.
