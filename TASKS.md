...
- [ ] `@minsky/token-monitor` — Maciek `claude-monitor` Strategy implementation
  ...
  - **Estimate**: 4–6h
  - **Hypothesis**: Maciek's `claude-monitor` exposes a parseable cache file whose format is stable enough that a thin adapter can derive a `TokenSnapshot` whose `tokensRemainingInWindow` agrees with `claude-monitor --json`'s reported value to within 1 % across 100 consecutive ticks.
  - **Details**: Real `TokenMonitor` Strategy against Maciek's Python `claude-monitor` cache file (path & format documented in their repo). Polls the cache, parses, returns `TokenSnapshot`. Adapter test runs against a real Maciek install (CI installs Maciek's pinned version once). **Preparation-PR pattern (rule #9):** before this task, ship a small preparation PR that adds the CI step `claude-monitor --version` and gates the adapter test on it. That PR's hypothesis: pinning Maciek's version in CI makes the adapter test reproducible.
  ...
  - **Measurement**: `pnpm vitest run novel/adapters/token-monitor/src/maciek.test.ts` exits 0 against the pinned Maciek install on CI; assertion uses `Math.abs(snapshot.tokensRemainingInWindow - cliJson.tokensRemaining) / cliJson.tokensRemaining < 0.01`.
  - **Pivot**: if Maciek's cache format changes more than once in a 90-day window after this task lands — pivot to a custom `TokenMonitor` Strategy that polls Anthropic's API directly (when / if Anthropic exposes a usage endpoint), or write our own cache by hooking the API client.
...
- [ ] Extract `selfTest()` contract to shared adapter-types package
  ...
  - **Estimate**: 2–3h
  - **Hypothesis**: Hoisting `SelfTestStatus`, `SelfTestResult`, and `aggregateStatus()` into a leaf `@minsky/adapter-types` package eliminates the latent dependency cycle (`@minsky/budget-guard → @minsky/observability` for a base type) without breaking observability's public API; future adapter packages then depend on `@minsky/adapter-types` directly.
  ...
  - **Measurement**: `pnpm typecheck && pnpm test && pnpm publish --dry-run --workspace novel/adapters/types` all exit 0; `node -e "console.log(require('@minsky/adapter-types').aggregateStatus(['green','red']))"` prints `red`; pre-task baseline `pnpm vitest run novel/adapters/observability/src/index.test.ts` count of tests is unchanged after the refactor.
  - **Pivot**: if extraction breaks observability's downstream consumers (the `@minsky/observability` re-export turns out not to preserve type identity across declaration files) → revert and instead inline the contract per-adapter with a CI lint that asserts the inline copies are byte-equal.
  - **Acceptance**: New `@minsky/adapter-types` package compiles, tests pass, observability re-exports work, setup.sh comment updated.
  - **Anchor**: Martin, *Clean Architecture*, 2017 (acyclic dependency principle); Wiggins, *The Twelve-Factor App*, 2011 (factor II — explicit dependencies).
