...
- [ ] `tick-loop-daemon-real-spawn` — drop the dry-run guardrail; spawn real `claude` / `omc /team` subprocesses
  - **ID**: tick-loop-daemon-real-spawn
  - **Tags**: novel, runtime, supervision, blocker
  - **Estimate**: 2–3d
  - **Hypothesis**: Replacing the dry-run guard in `runDaemon` (`novel/tick-loop/src/daemon.ts` — currently throws on `dryRun: false`) with a real `child_process.spawn('claude', […])` (or `omc /team`) call, threading `~/.claude/` config + `~/.zshenv` env-passing into the spawn opts, lets the daemon actually run tasks end-to-end. v0 (`tick-loop-daemon-v0`, shipped) ships ONLY the dry-run path because real spawn has un-mitigated risk surface (env-passing, stdin/stdout deadlock, lease-then-spawn race) that's safer to land in a separate, narrower follow-up. Replacing the guard makes the loop's MAPE-K monitor role go from `partial` (vision.md row 64 + 66) to `full`.
  - **Details**: 1) Replace the `if (!opts.dryRun) throw` guard with a `dryRun ? spawnTickDryRun(…) : spawnTickReal(…)` dispatch. 2) `spawnTickReal({ task, env })` shells `claude` (or `omc /team <persona>` once `persona-spawner-v0` ships) with the task's brief on stdin and the env explicitly populated from `~/.claude/` + `~/.zshenv`. 3) Wire the real `BudgetGuard.decide()` from `@minsky/budget-guard` (instead of the v0 stub `BudgetGuardLike`); `budget-guard-maciek-impl` is the precondition because the Strategy needs the real `TokenMonitor`. 4) Update `bin/tick-loop.mjs`: drop the hard-coded `--dry-run` arg in `distribution/systemd/run-tick-loop.sh` (or gate it on `MINSKY_TICK_DRY_RUN=1`). 5) Add an integration test that spawns a real `claude --resume` against a synthetic task in CI (skip in CI if `claude` unavailable; runs locally + on self-hosted runner per the parent's `first-integration-test` precedent).
  - **Files**: `novel/tick-loop/src/daemon.ts` (replace dry-run guard with dispatch), `novel/tick-loop/src/daemon.test.ts` (integration test, gated on `claude` presence), `novel/tick-loop/bin/tick-loop.mjs` (drop hard-coded `--dry-run`), `distribution/systemd/run-tick-loop.sh` (drop hard-coded `--dry-run`), `novel/tick-loop/package.json` (add `@minsky/budget-guard` dep), `vision.md` (collapse rows 64 + 66 + this row's row into a single full-MAPE-K-monitor row)
  - **Verification**: `bash distribution/systemd/run-tick-loop.sh --max-iterations=1` against a real synthetic task exits 0 with the task marked completed; integration test green on self-hosted runner; existing 13 dry-run tests still pass (the dispatch keeps the dry-run path).
  - **Measurement**: `pnpm vitest run novel/tick-loop/src/daemon.test.ts --reporter=json | jq -e '.numPassedTests >= 14 and .numFailedTests == 0'` exits 0 (one new integration test on top of the 13 dry-run tests); `gh run list --workflow ci.yml --limit 30 --json conclusion --jq '[.[] | select(.conclusion=="success")] | length' >= 28` (≥93 % green over 30 runs after merge).
  - **Pivot**: if `claude` doesn't expose a non-interactive mode that's compatible with the daemon's process-supervision model (e.g., refuses to read brief from stdin without a TTY), file an upstream feature request and pivot to a thin `claude --resume` chain managed by the daemon directly via a temp file; if THAT fails, pivot to file-based handoff (write brief to `state/inbox/<id>.md`, expect operator to consume — the parent task `tick-loop-daemon-v0`'s documented Pivot path).
  - **Acceptance**: daemon runs against real `claude` end-to-end on self-hosted runner; story-001's MTTR <5 min claim becomes measurable; the dry-run path remains for tests; vision.md row 64 + 66 + this PR's row collapse to a single full-conformance MAPE-K-monitor row.
...
- [ ] `user-story-001-integration-test-real` — actual integration test against the real daemon (not mock)
  ...
  - **Estimate**: 1d
  - **Blocked by**: tick-loop-daemon-real-spawn
