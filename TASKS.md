...
- [ ] `standing-audit-gap-loop` — observability-focused gap audit (read-only, surfaces tasks the system + observability layer expose)
  - **ID**: standing-audit-gap-loop
  - **Tags**: standing-loop, audit, observability
  - **Estimate**: 1–2h per pass
  - **Hypothesis**: Auditing the running daemon's observability surface (OTEL spans / OpenObserve dashboards / `/watch.json` / Apple Shortcuts watch surface) against the codebase's actual instrumentation reveals ≥1 actionable gap per pass: missing `@otel` JSDoc on exported functions, code paths that don't emit a span, dashboards/queries that reference metrics the code never publishes, watch-surface metric drift (`WATCH_METRIC_IDS` vs what `/watch.json` actually serves), or instrumentation that fires once but is never read. The pass is read-only: it converts gaps into queue items the daemon picks up next iteration, fueling the loop.
  - **Details**: Walk the audit cascade with an observability lens. (1) Tier-1 verify: re-run the test suite + typecheck on main. (2) Observability tier: cross-check `WATCH_METRIC_IDS` (`novel/dashboard-web/src/watch.ts`) against what `/watch.json` actually serves AND what the Shortcut JSON manifests in `distribution/shortcuts/*.shortcut.json` extract; cross-check OpenObserve dashboard queries (if any committed) against the spans/metrics actually emitted; rule-#4 lint already covers exported function `@otel` tags but doesn't cover internal helpers — surface those as P3 spans-to-add tasks; verify the OTEL spans fired by the daemon match the names referenced in vision.md row 67's "MAPE-K monitor" prose. (3) For each gap, write a P1/P2/P3 task with deterministic Measurement (e.g., `node scripts/check-rule-4-otel-coverage.mjs --diff-base=origin/main` or a grep). (4) Remove this `standing-audit-gap-loop` block in the same commit so the next pass is a fresh seed.
  - **Files**: `novel/dashboard-web/src/watch.ts`, `novel/dashboard-web/test/render.test.ts`, `distribution/shortcuts/*.shortcut.json`, `vision.md` § "Pattern conformance index" rows 56-67, `novel/observability/src/*` if relevant, `scripts/check-rule-4-otel-coverage.mjs` (audit it for false negatives)
  - **Verification**: at least one new P1/P2/P3 task is filed per pass, OR the commit message explicitly states "audit found no observability gaps" and removes the block.
  - **Measurement**: `git log --oneline main -1 | grep -E '(standing-audit-gap-loop|chore: audit)'` shows the commit; the queue grows by ≥1 task or the pass closes with the no-gaps commit.
  - **Pivot**: if 3 consecutive passes find zero observability gaps, retire this standing loop and replace it with a quarterly cadence (cron-driven) — surface drift would be slow at that point.
  - **Acceptance**: gap → task → next iteration picks it → PR ships → queue refills. Self-fueling loop visible in commit log over the 2h window.
  - **Anchor**: Kephart & Chess 2003 (MAPE-K — the *Knowledge* phase needs the Monitor surface to be honest); Beyer SRE 2016 Ch. 6 (monitoring distributed systems — every emitted metric must be read; every read must have a publisher); rule #4 (every novel exported function carries an `@otel` JSDoc — this audit exercises it).
  - **Risk**: noisy queue if every micro-gap becomes a task. Mitigation: only file if the gap blocks an observable behaviour (e.g., dashboard query references a span the code never emits) — pure aesthetic gaps go in a single P3 cluster task, not 20 separate ones.
...
- [ ] `daemon-budget-pause-observability` — surface `budget-paused` daemon state to operator (Ntfy push + `/watch.json` field)
  - **ID**: daemon-budget-pause-observability
  - **Tags**: novel, observability, runtime, surfaced-by-self-run
  - **Estimate**: 2–3h
  - **Hypothesis**: The first 2h live-on-itself run (started 2026-05-04 ~11:13 local) immediately surfaced this gap: the daemon's `iteration.status: "budget-paused"` (real `BudgetGuard.decide()` circuit-break, sub-task 2/3 of `tick-loop-daemon-real-spawn`) emits an OTEL span and writes to the local log, but has NO visible signal on the calm-tech surface — no Ntfy push, no `/watch.json` field, no Apple Shortcut indicator. Operator only sees "the daemon isn't doing work" via tail-the-log, which violates the watch-shortcuts vision (Card & Mackinlay 1999 — three numbers visible without effort). Adding (a) a Ntfy push on the *transition* into budget-paused (not on every iteration), (b) a `paused: { reason: "budget" | "operator" | null, since: ISO }` field on `/watch.json`, and (c) a Shortcut surface tile that flips colour on non-null `paused.reason` closes the loop. The Apple Shortcut for `pause` already wires the `state/PAUSED` sentinel — this task ADDs the operator-INFORMING half: when the daemon pauses ITSELF, the operator's wrist sees it.
  - **Details**: 1) Extend the daemon's `runDaemon` to detect the *first* tick where `iteration.status === "budget-paused"` and call `notifier.push({ kind: "budget-paused", reason })` exactly once (debounce on transition; don't spam every 5min). Re-arm the trigger when status returns to a non-paused state. 2) Add a `paused` field to `WatchSnapshot` (`novel/dashboard-web/src/watch.ts`); populate it from a small `paused-state.json` (or in-memory if the daemon shares process — clarify which). 3) Update `WATCH_METRIC_IDS` if needed (it already caps at 3; this is a 4th-channel boolean, not a 4th metric — clarify in docs that the 3-value cap is for *numeric* metrics, the boolean control flag is the existing `paused` already there). 4) Verify the existing 5 polling Shortcuts can render the `paused` field (already a single-bool channel per row 65). 5) Add an integration test that asserts: budget-paused → exactly 1 push call (StubNotifier), paused field flips to `{reason: "budget"}`, recovery → push count stays at 1, paused field flips back to null.
  - **Files**: `novel/tick-loop/src/daemon.ts`, `novel/tick-loop/src/daemon.test.ts`, `novel/dashboard-web/src/watch.ts`, `novel/dashboard-web/test/render.test.ts` or `test/server.test.ts`, `vision.md` row 67 (note the budget-pause signal join), possibly `distribution/shortcuts/last-task-status.shortcut.json` if the existing Shortcut covers this
  - **Verification**: integration test asserts: budget-paused emits exactly 1 push (debounce holds across N iterations), `/watch.json` `paused.reason === "budget"`, recovery emits 0 additional pushes and clears the field.
  - **Measurement**: `pnpm vitest run novel/tick-loop novel/dashboard-web --reporter=json | jq -e '.numPassedTests >= 36 and .numFailedTests == 0'` (today: 34 in tick-loop + ~25 in dashboard-web; +1-2 new).
  - **Pivot**: if Apple Shortcuts can't render a 4th boolean cleanly given vision row 12's "three numbers" cap, ship just (a) Ntfy push + (b) `/watch.json` field; defer Shortcut visualization to a P3 follow-up (`watch-paused-tile`). The push alone closes the operator-informing gap.
  - **Acceptance**: budget-pause transitions emit exactly 1 Ntfy push (not N); `/watch.json` exposes the pause reason and timestamp; integration test pins the debounce; vision row 67 documents the operator-informing path.
  - **Anchor**: Card, S. K., Mackinlay, J., Shneiderman, B., *Readings in Information Visualization*, Morgan Kaufmann, 1999 (3-number glanceable display + state-change indicators); Beyer SRE 2016 Ch. 6 (every internal state operator cares about must surface — silence is failure); Kephart & Chess 2003 (MAPE-K — Knowledge phase requires Monitor surface to be honest about Plan-time gating).
  - **Risk**: push spam if the debounce is wrong. Mitigation: the integration test fixes the count exactly to 1 per transition, asserting the debounce is mechanical, not best-effort.
  - **Surfaced-by**: 2026-05-04 11:13 local — first live `bash distribution/systemd/run-tick-loop.sh --max-iterations=24 --tick-interval-ms=300000` run from main `4bdc2b7` immediately reported `iteration.index=0 status=budget-paused` from the tick-loop log; operator (the user) had to manually `tail` the log to know. Exactly the signal the watch surface should make automatic.
