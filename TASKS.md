...
- [ ] `@minsky/token-monitor` — Maciek `claude-monitor` Strategy implementation
  ...
  - **Files**: `novel/adapters/token-monitor/src/maciek.ts`, `novel/adapters/token-monitor/src/maciek.test.ts`, `novel/adapters/token-monitor/test/fixtures/claude-config-snapshot/` (committed Anthropic config-dir fixture); upstream reference: `claude_monitor/data/reader.py` (`~/.claude/projects/**/*.jsonl` glob), `claude_monitor/core/data_processors.py` (`TokenExtractor.extract_tokens` — reads `message.usage.{input_tokens,output_tokens,cache_creation_input_tokens,cache_read_input_tokens}`), `claude_monitor/data/analyzer.py` (5h `SessionBlock` rounding rule), `claude_monitor/core/plans.py` (`PLAN_LIMITS` token caps: pro=19_000, max5=88_000, max20=220_000, custom=44_000).
  ...
  - **Research**: 2026-05-04 — Anthropic config-dir format (read-only, from `claude-monitor==3.1.0` wheel + live filesystem inspection). **Correction to prior Hypothesis/Details**: Maciek does NOT read `~/.config/claude/`; that env-var path (`CLAUDE_CONFIG_DIR`) only governs `claude-monitor`'s own settings cache. The actual data source is `~/.claude/projects/**/*.jsonl` — Claude Code writes one JSONL file per session (filename = session UUID), grouped under one directory per cwd (directory name = cwd path with `/` → `-`, e.g. `~/.claude/projects/-Users-cbrwizard-apps-minsky/<uuid>.jsonl`). Each line is a JSON message; assistant turns carry `message.usage` with shape `{input_tokens, output_tokens, cache_creation_input_tokens, cache_read_input_tokens, service_tier, cache_creation: {ephemeral_1h_input_tokens, ephemeral_5m_input_tokens}, ...}`. **`tokensRemainingInWindow` is computed, not stored.** Algorithm (Maciek's `data/analyzer.py` + `core/plans.py`): (1) `rglob` all `.jsonl` under projects/, deduplicate by `(message.id, requestId)`; (2) group entries into 5h `SessionBlock`s — block start = `floor(first-entry-timestamp, hour)` UTC, end = start+5h, new block when entry.ts ≥ block.end_time OR gap-since-last-entry ≥ 5h; (3) the *active* block is the one whose `[start, end)` contains `now()`; (4) `tokens_used = sum(input + output + cache_creation + cache_read)` over that block's entries; (5) `windowSizeTokens` ← plan limit (`pro=19_000`, `max5=88_000`, `max20=220_000`, `custom=44_000`) OR observed peak across history if plan unknown; (6) `tokensRemainingInWindow = max(0, windowSizeTokens − tokens_used)`; (7) `secondsUntilWindowReset = (block.end_time − now()).total_seconds()`. **Pollable values** (all derived; no `current_window_start` / `tokens_used` / `window_max` are persisted by Anthropic): the latest entry timestamp + the rolling per-block sum recomputed on each tick. **v0 fixture sketch** at `novel/adapters/token-monitor/test/fixtures/claude-config-snapshot/`: a synthetic `~/.claude` subtree — `projects/-fixture-cwd/aaaa-bbbb-cccc.jsonl` containing ~5–10 redacted assistant entries spanning a known 5h window, plus a sibling `expected.json` carrying `{ now: "2026-05-04T12:00:00Z", plan: "max5", tokensUsed: <N>, tokensRemainingInWindow: <88000-N>, secondsUntilWindowReset: <S>, maciekRealtimeReportedRemaining: <88000-N> }` so the parser test asserts deterministic counts without invoking Maciek. **Parser shape** for `MaciekTokenMonitor.snapshot()`: `(opts: { configDir: string, now?: () => Date, plan?: 'pro'|'max5'|'max20'|'custom' }) → Promise<TokenSnapshot>` — rglob `<configDir>/projects/**/*.jsonl`, stream-parse with line-by-line `JSON.parse` (skip blanks; tolerate `JSONDecodeError` per Maciek), filter to `m.message?.usage` with `input_tokens>0 || output_tokens>0`, dedup by `(message.id, requestId)`, group into 5h blocks via the analyzer rule above, pick the active block, sum tokens, subtract from plan cap → return `{ tokensRemainingInWindow, windowSizeTokens, secondsUntilWindowReset, weeklyHeadroomFraction: null /* deferred — Maciek's P90 ML predictor not exposed without their CLI */, observedAt: now().toISOString() }`. The `node:fs` watch hook (`fs.watch(projectsDir, { recursive: true })`) is a fast-path for re-snapshot; default is poll-on-`snapshot()`.
