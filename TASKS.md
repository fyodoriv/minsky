...
- [ ] `rule-11-flake-detection-lint` — auto-detect flaky CI gates and downgrade them per rule #11
  ...
  - **Details**:
    ...
    2. Implement `gh run list` ingestion as the I/O boundary, with rule-#9 timeout cap (30 day window per query).
    3. Patch emission: for each FlakeReport, emit `tmp/rule-11/<job>-task-block.md` (the TASKS.md insertion) and `tmp/rule-11/<job>-workflow.patch` (the workflow downgrade). Idempotent — re-running the lint when patches already exist is a no-op.
    4. Wire as a CI job in `ci.yml` (read-only — never auto-pushes patches; just surfaces them in `tmp/rule-11/` as workflow artifacts the operator inspects).
    5. Land paired tests: synthetic 30-day run history with one job at 11 %, two below 10 %; assert exactly one FlakeReport surfaced; assert idempotent re-run.
    6. Add vision.md row in same commit per rule #8.
  - **Files**: `scripts/check-rule-11-no-flaky-gates.mjs` + `.test.mjs`, `.github/workflows/ci.yml` (new job entry), `vision.md` (new pattern-conformance row), `tmp/rule-11/.gitkeep` (artifact directory).
  - **Verification**: paired tests cover the threshold boundary (10 % vs 11 %), the same-SHA pairing detection, idempotent re-runs, and the patch-emission shape. CI lint runs on every PR and emits artifacts only when flakes are detected.
  - **Measurement**: `pnpm vitest run scripts/check-rule-11-no-flaky-gates.test.mjs --reporter=json | jq -e '.numFailedTests == 0 and .numPassedTests >= 6'` exits 0. AND `node scripts/check-rule-11-no-flaky-gates.mjs --dry-run --fixture=test/fixtures/30-day-history.json` produces exactly the expected `tmp/rule-11/*` files for the fixture.
  - **Pivot**: if `gh run list` rate-limits the ingestion (the 30-day-window query is heavy across N workflows), pivot to *cached* ingestion: a separate `flake-stats-collector.yml` workflow runs nightly, writes to `flake-stats/<workflow>-30d.json`, and the lint reads from the cached file rather than calling `gh` directly. This pivot trades freshness (up-to-24h-stale stats) for rate-limit safety; the trade-off is acceptable because the stop-the-world rule is honoured by the per-PR detection on the *single* failed-then-passed-on-rerun observation, not just the 30-day rate.
  - **Acceptance**: lint runs on every PR; emits patches when flakes detected; ≥6 paired tests cover the decision function; vision.md row added; the lighthouse-mobile downgrade shipped 2026-05-04 becomes the canonical "what the lint emits" example.
  - **Anchor**: Memon, A., et al., "Taming Google-Scale Continuous Testing", *ICSE-SEIP* 2017 (the "DETECTOR" system at Google — the per-job flake-rate primitive this lint implements is exactly the mechanism Memon et al. describe at scale); Forsgren, Humble, Kim, *Accelerate*, 2018 Ch. 3 (DORA test-reliability — flaky gates teach the team to ignore failure); Beck, *Extreme Programming Explained*, 1999, Ch. 17 (CI as the constraint enforcer — the flake-detection lint is itself a constraint); rule #10 (deterministic enforcement); rule #11 (no flaky metric is a load-bearing claim — this lint is rule #11's mechanical ratchet, the difference between "we say flakes are forbidden" and "we mechanically catch them").
  - **Failure modes**:

    | Failure mode | Trigger / fault axis | Expected behavior | Chaos test |
    |---|---|---|---|
    | `gh run list` returns truncated history (<30 days) for a busy workflow | upstream-API-limit | the lint reports `coverage: <N> days available, threshold check applied with a confidence asterisk`; emits a softer warning rather than an auto-downgrade patch | `scripts/check-rule-11-no-flaky-gates.test.mjs` — fixture with 7-day history, assert the partial-coverage path |
    | Two PRs ship the same flake fix simultaneously | concurrency | the lint is idempotent — second emission no-ops because the patches at `tmp/rule-11/<job>-*` already exist; the operator merges one, the other becomes a stale artifact cleaned by a follow-up cron | covered by the idempotent-rerun test |
    | A flake is intermittent — 11 % over 30 days but 5 % over 7 days | windowing | the rule's threshold is the 30-day window precisely so short-term variance doesn't trigger; the per-PR single-observation path catches the rest | covered by the windowing tests |
    | The lint itself becomes flaky (gh API rate-limit causes inconsistent verdicts) | meta-flakiness | rule #11 applies recursively — the lint's own CI job is subject to the same rule; if it flakes ≥1 / 10, the auto-detection ratchet downgrades *itself* to informational and a P0 surfaces calling out the meta-issue | filed in the lint's own EXPERIMENT.yaml as a pre-registered pivot path |
