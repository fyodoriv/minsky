...
- [ ] `user-stories-dual-purpose-refresh` — refresh user-stories/001 + 006 to explicitly frame Minsky-on-itself + Minsky-on-any-host as the same loop
  - **ID**: user-stories-dual-purpose-refresh
  - **Tags**: docs, user-stories, vision
  - **Estimate**: 1h
  - **Hypothesis**: The two flagship user stories (`001-loop-runs-overnight.md` for Minsky-on-itself; `006-runner-on-any-repo.md` for Minsky-on-any-host) currently read as independent stories; the user's 2026-05-04 brief restated the original vision as "Minsky transforms any repo, including itself, as the *same* loop". Refreshing both stories' opening framing — explicitly cross-referencing the dual-purpose surface, citing rule #12, and noting that the Minsky-on-itself surface is a *side effect* of the same supervisor that Minsky-on-host runs — closes the dual-purpose framing gap without rewriting the substantive acceptance criteria.
  - **Details**: (a) `user-stories/001-loop-runs-overnight.md`: add a "Dual-purpose framing" section near the top noting that this story is "Minsky on itself" and links to 006 as the cross-repo counterpart; cite rule #12. (b) `user-stories/006-runner-on-any-repo.md`: same treatment, swapped. (c) Update vision.md's § "What Minsky is" cross-reference if needed (this PR's vision update already references both — verify links resolve).
  - **Files**: `user-stories/001-loop-runs-overnight.md`, `user-stories/006-runner-on-any-repo.md`.
  - **Verification**: both stories explicitly state the dual-purpose framing within their first three paragraphs; both cite rule #12; both link to the other.
  - **Measurement**: `grep -c "rule #12" user-stories/001-loop-runs-overnight.md user-stories/006-runner-on-any-repo.md` returns ≥1 per file. AND `grep -E "(006-runner-on-any-repo|001-loop-runs-overnight)" user-stories/001-loop-runs-overnight.md user-stories/006-runner-on-any-repo.md` returns ≥1 cross-link per file.
  - **Pivot**: if the dual-purpose framing reads as redundant (two stories saying "we are the same loop" creates confusion), retire the cross-link in favour of a single *umbrella* user-story — `user-stories/000-minsky-transforms-any-repo.md` — that frames both as instances of one pattern, with 001 / 006 reduced to acceptance specifications.
  - **Acceptance**: both stories cross-reference each other + rule #12; readers understand the dual-purpose framing without re-reading vision.md.
  - **Anchor**: rule #12 (this PR's vision update); rule #5 (clear vocabulary — the dual-purpose framing is a vocabulary item).
  - **Risk**: low. Pure docs change.
