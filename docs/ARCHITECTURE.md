...
| 17 | Sandbox shape | `SandboxAdapter` (`novel/adapters/sandbox/`) | host-spawn via `node:child_process` (`novel/adapters/sandbox/src/process-sandbox-adapter.ts` — today's behavior, the default) | dockerode, remote-vm-over-ssh | Low — the host-spawn Strategy is the always-available default with no external daemon; the abstraction mirrors OpenHands' `app_server/sandbox/` three-shape pattern. The Docker Strategy is the sibling `research-finding-docker-sandbox-adapter` task; the Remote-VM Strategy is future M4 work |
