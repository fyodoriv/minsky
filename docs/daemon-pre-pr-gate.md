...
## Two drift hazards, two mitigations

The gate's value depends on two parity claims, each with its own pin:
...
The gate is now invariant to which transport invokes it (operator terminal, lefthook pre-push, daemon iteration).
