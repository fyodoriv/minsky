...
8. **Lefthook stage selection parity.** The slice-4 lefthook test pins that `pnpm pre-pr-lint` appears under `pre-push:` — but it passes regardless of which `--stage` argument follows (or none). A future PR that flips `--stage=full` to `--stage=fast` (the daemon's stage) silently undergates the operator's `git push` against every full-only step (`vitest`, the rule-#9 record lints, `pattern-index`, the dormant-config caps, etc.) — vitest failures defer to CI instead of being caught locally, which is exactly the operator-side babysitting the task block tries to eliminate. The doc's "Operator commands" section already says `pnpm pre-pr-lint --stage=full` is "what lefthook pre-push runs" — slice 20/N's `lefthook.yml pre-push runs --stage=full` test in the `lefthook pre-push contract` block pins the claim. Mutation-tested: rewriting the lefthook arg to `--stage=fast` (or dropping the flag) fails the test.

The gate is now invariant to which transport invokes it (operator terminal, lefthook pre-push, daemon iteration), to which name a fast-stage step takes (brief and manifest stay in lockstep), to where the aggregator gate is sourced (the `needs:` declaration and its bash gate-check stay set-equal), to which stage a doc bullet documents (the `pnpm pre-pr-lint --stage=full` set is equally pinned), to which CI jobs the manifest intentionally omits (the env-dependent allowlist's enumeration in code and in the doc stay set-equal), and to which stage the operator's `git push` actually runs (the lefthook arg is pinned to `--stage=full`).
