...
`fresh-clone-smoke` is registered in `SEPARATE_WORKFLOW_GATES` (not `CI_ENV_DEPENDENT_JOBS`) because it lives in `.github/workflows/fresh-clone.yml` — its own workflow file, not the `ci.yml` aggregator. It destroys `novel/tick-loop/dist/` to simulate the stale-build path; running it locally would wipe the dev build.
