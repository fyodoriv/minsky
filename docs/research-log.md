...
### 2026-Q3 — Quarterly dependency review (review-q3-2026)

Resolves `review-q3-2026`. Every dependency-table row (`ARCHITECTURE.md` § "The dependency table") and every novel layer (`ARCHITECTURE.md` § "The novel layers") is reconsidered against rule #1 (don't reinvent the wheel) using the pre-2026-Q3 audit baseline above (2026-05-04 `pnpm outdated` snapshot) as the before/after anchor. One bullet per layer; `keep` = the chosen building block is still the right one, `keep (bump)` = same tool, version drift to schedule, `watch` = a material change to monitor. Material changes are filed as separate TASKS.md task blocks (see "Material changes filed" at the end).

Dependency-table rows (18):

- **Orchestrator (OMC v4.13.x)** — keep. Still the most mature multi-agent orchestrator in the Claude Code ecosystem; no replacement candidate (claude-flow, MS Agent Framework, custom) closes the gap. No material change since the 2026-05-03 first pass.
- **InnerLoop (OMC Ralph mode + Anthropic ralph-wiggum plugin)** — keep. The ralph technique is canonical and now formalised by Anthropic's official plugin; frankbria/ralph-claude-code remains the safety-railed fallback. No material change.
- **TaskQueue (tasks.md + `@tasks-md/cli` + tasks-mcp, self-owned)** — keep. Single-file P0–P3 design still the deliberate divergence from directory-of-files alternatives (beads, taskmd-driangle). No material change; the cross-language-port open question stays open (no Python/Rust `tasks-md` package as of this review — see Open questions).
- **RoamCoordinator (tasks.md `/next-task` Roam step, self-owned)** — keep. Novel to tasks.md; pace of change is ours. No material change.
- **TokenMonitor (Claude-Code-Usage-Monitor / Maciek-roboblog, pinned `claude-monitor==3.1.0`)** — keep (bump). Still the most feature-complete OSS option; the 2026-05-03 finding (no `--json` flag) stands, so the `budget-guard-maciek-impl` adapter reads upstream's `~/.config/claude/` data source directly rather than the CLI stdout. Verify the pinned CI version against the latest PyPI release when `budget-guard-maciek-impl` is implemented.
- **LocalDashboard (claude-dashboard / seunggabi)** — keep. Closest existing match for "all sessions in one place"; naming collision with schmoli/claude-dashboard remains the only watch-item (source is pinned). No material change.
- **MobileDashboard (claude-code-monitor / onikan27)** — watch. The macOS-only AppleScript dependency still blocks Linux users; the long-term preference (custom Hono web app, ~300 LOC) is unchanged. Decision still pending per the open TASKS.md item — no new information forces it this quarter.
- **RemoteAccess (Tailscale, free tier)** — keep. Industry-standard mesh VPN; closed control plane / OSS data plane trade-off accepted (same carve-out as the dep table notes). No material change.
- **Notifier (ntfy.sh)** — keep. Simplest pub/sub HTTP push with iOS→Apple-Watch propagation; Pushover/Telegram remain heavier fallbacks. No material change.
- **WatchActions (Apple Shortcuts)** — keep. The cheap three-number surface still meets story 005's calm-tech invariant; the native-WatchOS escalation is pre-specified (§ "Native WatchOS app") behind a 90 s/day wrist-dwell trigger that has not fired. No material change.
- **Ollama (HTTP `/api/generate` + `/api/ps`, since 2026-05-29)** — keep. `keep_alive` eviction primitive stable since v0.1.x; the LiteLLM-sets-`keep_alive` regression is caught by the `ollama-daemon-idle-wired-memory-mb` metric within 14 days. Newest in the table; no drift to act on.
- **Supervisor (systemd / launchd, OS-native)** — keep. Erlang/OTP-style supervision maps cleanly onto OS service managers; no extra runtime justified. No material change.
- **Observability (Claude Code OTEL → local Loki/Tempo/Prometheus/Grafana)** — keep (watch). The 4-service local stack is still heavy; the resolved recommendation (OpenObserve, § "Lighter OTEL backend") stands but the `observability-adapter-v0` swap has not landed, so the `Current` cell remains the Grafana stack. Watch: revisit once the lighter-backend adapter ships.
- **PromptOptimizer (DSPy + Promptfoo)** — watch. DSPy was rejected as the implementation (§ "DSPy fit": Python-only runtime + 5:4 friction ratio); the fallback TypeScript interface ships against the Anthropic SDK when `mape-k-loop-v0` lands. The `Current` cell will move then. No new information overturns the rejection this quarter.
- **SpecMonitor (custom Claude Skill, advisory-only per rule #10)** — keep. Wholly novel; the load-bearing share is the deterministic `scripts/check-rule-{1..7}-*.mjs` linters and the residual judgement is the ≤5-rule advisory Skill. No off-the-shelf runtime spec-monitor for LLM-agent behaviour has appeared. No material change.
- **A2A (`novel/adapters/a2a/`, A2A v1.0.0 / Linux Foundation)** — keep (watch). Cross-vendor industry standard; the real google-a2a-python bridge is gated on the 2026-06-01 OpenHands runtime, so the scaffold reports `yellow` (never a false `green`). Watch: confirm the bridge flips live now that the gate date has passed, and re-evaluate AGNTCY if it gains Linux-Foundation backing.
- **MCP (`novel/adapters/mcp/`, MCP v2025-11-25 / Anthropic)** — keep (watch). De-facto agent-to-tool standard with the largest public server ecosystem; the `@modelcontextprotocol/sdk` bridge is gated on the same 2026-06-01 OpenHands runtime. Watch: the v2026-07-28 RC migration is the sibling `mcp-migration-to-2026-07-28-rc` task; version shims live below the 3-verb interface seam.
- **SandboxAdapter (`novel/adapters/sandbox/`, Docker CLI via `child_process`)** — keep. Opt-in via `untrusted: true` in `.minsky/repo.yaml`, off by default; reports `yellow` and falls back to the bash-runner when the Docker daemon is absent. Podman/gVisor/Firecracker remain swap-in Strategy candidates. No material change.

Novel layers (5):

- **`claude-budget-guard`** — keep. The token-budget watchdog (flag file + JSON API) reads from the `TokenMonitor` adapter and is independent of OMC/tasks.md; still the right small extractable package. Extraction target unchanged.
- **`claude-handoff-spec`** — keep. The persona-to-persona handoff spec (Status/Summary/Artifacts/Blockers/Suggested-next/Pushback) plus reference parser + validator has no off-the-shelf equivalent; the community-effort play (make handoff format the way tasks.md is becoming for queues) stands. Extraction target unchanged.
- **`claude-spec-monitor`** — keep. Split per rule #10's ratchet: deterministic linters carry the load-bearing share, the advisory Skill carries the residual judgement (≤5 rules, never gating). Each new deterministic linter retires its matching Skill check. No material change.
- **`claude-mape-k-loop`** — keep. The autonomic manager (Kephart & Chess 2003) is still pre-`v0`; the cadence design (§ "MAPE-K cadence", hybrid time + event + tick-backstop) is committed but unmeasured. The "replace estimates with observed values once `mape-k-loop-v0` ships" open question carries forward. No material change.
- **`omc-tasksmd-bridge`** — keep. v0 shipped read-only (OMC → tasks.md) per the resolved OMC-handoff-persistence finding; its success metric ("this package becomes unnecessary") is unchanged — file/track the upstream OMC native-tasks.md issue. No material change.

Rule-#9 prediction calibration (Q3 2026): **0 experiments with observed deltas tracked in the production experiment store** (`.minsky/experiment-store/cross-repo/*.jsonl` holds only test fixtures; the MAPE-K `v0` loop that writes real prediction-vs-observation records has not shipped). Validated 0% / regressed 0% / inconclusive 0% — denominator is 0. Mean predicted-vs-observed delta by hypothesis category: **not yet computable** — every threshold in this file is still annotated `estimate`, consistent with the file's own "replace estimates with observed values once `mape-k-loop-v0` ships" framing. No rule-#9 amendment is proposed because there is no observed miscalibration to correct yet; the first non-trivial calibration line is owed at the 2026-Q4 review *iff* `mape-k-loop-v0` has shipped and produced ≥1 verdict by then. This zero-baseline is itself the pre-registered before-anchor for that future delta.

Material changes filed: none reach the P1/P2 bar this quarter — every row is `keep` or `watch`, and each `watch` item is already tracked by an existing TASKS.md block (`observability-adapter-v0`, `mape-k-loop-v0`, the MobileDashboard decision, `mcp-migration-to-2026-07-28-rc`) rather than a newly-discovered change. Per the Pivot clause, this is review #1 of the "3 consecutive zero-material-change reviews → drop to semi-annual" counter (the 2026-05-03 and 2026-06-02 entries were a first-pass and a security-forced bump, not steady-state quarterly scans, so the counter starts here).
