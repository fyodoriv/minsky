...
Concrete worked example for the example-capabilities template (`## Why → ## What changed → ## How to test → ## Screenshots / recordings → ## IXPs → ## JIRA → ### PR checklist`):
...
If the PR changes pixels — UI, design system, story snapshots, dashboard, generated docs site — the screenshot or before-after pair goes in the repo template's Screenshots section (most repos have one; example-capabilities calls it `## Screenshots / recordings`). The TL;DR carries a one-sentence summary of what the screenshot proves so a reviewer who only reads the lede still understands the visual change.
...
Bad title (the first commit's subject, PR has since grown to add Storybook + JSDoc): `test(example-ai-native): block CC tools with empty icon strings PROJ-994`

Good title (reflects full PR scope): `test(example-ai-native): vitest + storybook gates for cc icons PROJ-994`

### Canonical PR-body skeleton

A reader-priority PR body whose `## section` order matches the repo template exactly, with the reader-priority lede / inverted-pyramid rules applied inside each section. The example below uses the example-capabilities template (`Why → What changed → How to test → Screenshots → IXPs → JIRA → PR checklist`); substitute your repo's actual template section names + order.
...
supporting visuals. Use the github.example.com /raw/<branch>/<path>
...
PR-description section sourced from 2026-05-25 operator directive: "all PRs you create or update strictly follow great useful order of info for cold readers. Eg don't spam readers with technical details until you give them an overview." Surfaced by example-capabilities PR #2095 (https://github.example.com/example-org/example-capabilities/pull/2095) — agent shipped a PR body that opened with a `## Why this is needed` heading followed by a numbered list of failure modes, with no TL;DR sentence between the title and the technical analysis. Reviewer cold-read fail. The PR-body skeleton, the cold-reader contract table, and PR Rules 1–6 are the fix.
