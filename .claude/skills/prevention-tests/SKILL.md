...
Worked example (Command Center icon registry, AIFN-994 — 4 layers):
...
The full pattern shipped as iep-capabilities PR #2095 (AIFN-994 — "block CC tools with empty icon strings"):
...
PR: https://github.intuit.com/expertnetwrk-portal/iep-capabilities/pull/2095
...
Operator directive that crystallised the skill (2026-05-25): "Amazing, now this is kind of tests I expect when we need something to be prevented" — applied to the AIFN-994 spec after two rounds of strictness iteration (initial 3-layer gate → added PascalCase layer + diagnostic matchers + 4 tripwires).
