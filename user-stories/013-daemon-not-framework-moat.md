...
| Operator wants to "extend" Minsky by adding their own personas | Operator drops a custom persona file into `.minsky/personas/` | `graceful-degrade` — persona is recognized via the existing persona-spawner adapter (`novel/adapters/persona-spawner/`); operator extends WITHOUT writing code in the host repo, only declarative YAML in `.minsky/` | `test/chaos/custom-persona-via-yaml.test.ts` |
