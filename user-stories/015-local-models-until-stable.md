...
> Run Minsky entirely on a model on your own computer, with no cloud API key, until Minsky proves it can stay up on its own.

Minsky is a background program that does coding work for you while you are away. It runs as a *daemon* — a background program that keeps running on your machine, survives a terminal close, and restarts on crash. Each round of work is one *iteration*: pick a task, ask a coding assistant (the *agent*) to do it, capture the result, open a draft for you to review. The agent here is the coding assistant Minsky drives; Minsky orchestrates the agent, it is not the agent itself.

This story records a decision about which agent Minsky uses by default. The decision: until the daemon is reliably stable, Minsky runs every iteration against a model on your own computer (via Ollama, LM Studio, or MLX) and never requires a cloud API key. This is not a backup plan for when the cloud is unavailable. It is the main path, on purpose, until the operator — the human who runs Minsky, that's you — decides otherwise.

**Operator stance, recorded 2026-05-24.** The stance holds until the daemon's stability reaches 90%. Stability is measured by the M1.1 target: the rate of `spawn-failed-exit-minus-one-silent-empty-stderr` failures (a `spawn-failed` verdict — the agent process could not start or exited without producing output) plus the broader 24/7 pillar's restart-event rate. Only at 90% does it become *acceptable* to flip the default back to a cloud model (Claude, GPT, or Gemini) for your main loop. Cloud-model A/B benchmarks are still allowed while the stance is active — they are how the eventual flip gets measured — but they are not the daemon's default runtime path.

## Story

I am an operator with no Anthropic, OpenAI, or Gemini key. I have Ollama running on my MacBook with `qwen3-coder:30b` pulled. I want Minsky to drain my `TASKS.md` — the plain-text Markdown to-do list at my project's root — overnight, without handing me a cloud bill.
...
I run `minsky`. The daemon wakes on its timer and starts an iteration. The OpenHands shim — the wrapper that lets Minsky drive the OpenHands agent — spawns against Ollama. Draft PRs land. No cloud key ever materialized.

That is *the* path until I, the operator, say otherwise. The daemon does three things it must NOT do:

- It does not silently upgrade to a cloud model when it detects a key in my environment.
- It does not nag me to set `ANTHROPIC_API_KEY`.
- It does not slot a "use cloud for hard tasks, local for easy ones" auto-router in front of me.

It runs local until the M1.1 stability gate trips.

When the gate trips — `scripts/measure-stability.mjs` reports that 90% of the trailing-7-day iterations had a clean exit — the daemon does two things, once: it emits an observability event (`local-models stance: gate lifted`) and writes a banner to `minsky watch` recommending an A/B against a cloud model. The operator then decides whether to flip. The stance is *advisory* after the gate lifts and *iron* before it.
...
- **Definition**: Fraction of Minsky iterations in the trailing-7-day window where the operator's daemon ran against a local model AND the iteration exited cleanly (no `spawn-failed-exit-minus-one-silent-empty-stderr`-class failures, no supervisor-restart events from agent-runtime crashes). The *supervisor* is the outer watchdog (systemd on Linux, launchd on macOS) that restarts Minsky if it dies.
...
6. Run `minsky doctor` and assert exit 0 (an absent cloud key is not a doctor failure).
...
- The operator's directive from the 2026-05-24 chat session (preserved in `docs/validated-learnings.md` under `operator-vision-2026-05-22-canonical`): "For now until we're super stable we'll rely on local models instead of getting an anthropic key."

## Failure modes & chaos verification

**Steady-state hypothesis**: with a local model configured and Ollama reachable, the daemon runs iterations and ships draft PRs without ever reading a cloud key.
...
This story follows pre-registered hypothesis-driven development (rule #9): every change states its hypothesis, success threshold, pivot threshold, measurement command, and literature anchor before code is written.
...
- Pairs with story 014 (launcher-agnostic feature parity): because the runtime defaults to local, the launcher matters even less for runtime behavior — there is no cloud-key environment variable for a launcher to leak.
- Pairs with story 008 (per-task backend and personas): per-task backend selection still works; this story does not restrict it. It only sets the default to local.
