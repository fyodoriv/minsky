...
5. `scripts/measure-stability.mjs` is filed as a P0 follow-up task (`local-models-stability-gate-90-percent`) — it parses `.minsky/iterations.jsonl` over the trailing-7-day window and computes the clean-exit fraction. When ≥90%, it emits the "gate lifted" banner.
