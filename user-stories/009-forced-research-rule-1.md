...
> **Why this story exists.** Motivation bullet #4 in [README.md § "Why Minsky?"](../README.md#why-minsky): *"the 100th time an agent reinvents the wheel is the 100th time a human says 'why didn't you just use lodash?'"*. The operator-facing promise is that agents are mechanically prevented from shipping a bespoke implementation when an established library would do; this story documents the mechanism, its acceptance criteria, and the chaos coverage.

## Story

As a maintainer, I review a draft PR from a Minsky-spawned agent that added a new utility module under `novel/foo/`. The agent reached for a custom implementation of CSV parsing. Pre-pr-lint fails with:
...
The PR cannot merge. The agent reads the lint output, edits `novel/foo/README.md` to add `## Alternatives considered` with three rows (`csv-parser` — rejected because too large; `papaparse` — rejected because browser-only; node's native `readline` — rejected because no quote-handling), then re-pushes. Lint passes. The maintainer sees the research trail in the PR and can verify the agent actually evaluated the alternatives — not just rejected them in prose.
...
- **Steady-state hypothesis**: zero unjustified `novel/*` packages on `origin/main` at any time. The linter is the mechanical enforcement; the steady state is empty rejection queue + clean main.
...
- **Operator escape hatch**: add the package to the linter's allowlist (a short list in `scripts/check-rule-1-novel-justification.mjs` keyed by package name + a TODO note pointing at the backfill task) with an explicit `// rule-1-grandfathered: <reason> + task id` comment.
