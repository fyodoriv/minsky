...
// (which would include foreign plists like com.minsky.oncall-hub-plugin
...
// failing with "com.minsky.oncall-hub-plugin.plist failed to bootstrap
...
describe("setup.sh: bootstrap loop iterates rendered plists only", () => {
  ...
  test("comment documents the orphan-plist regression this fixes", () => {
    ...
    expect(src).toMatch(/com\.minsky\.oncall-hub-plugin\.plist/);
  });
});
