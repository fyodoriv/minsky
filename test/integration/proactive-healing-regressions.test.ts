...
describe("rule #17 — GH_HOST is resolved from git remote, never hardcoded", () => {
  ...
  test("github.example.com remote → GH_HOST=github.example.com", () => {
    const r = resolveGhHost({
      envGhHost: undefined,
      gitRemoteUrl: "git@github.example.com:team/repo.git",
    });
    expect(r).toEqual({ host: "github.example.com", source: "git-remote" });
  });
  ...
  test("regression: minsky-run.mjs imports resolveGhHost (not just exports it)", () => {
    ...
    // Hardcoded github.example.com must NOT survive in the daemon's call sites.
    // The string may appear in a comment explaining what was removed; the
    // banned shape is "GH_HOST: 'github.example.com'" or
    // `GH_HOST = "github.example.com"` — never assigned as a default.
    expect(src).not.toMatch(/GH_HOST\s*[:=]\s*['"]github\.example\.com['"]/);
    expect(src).not.toMatch(/GH_HOST\s*\?\?\s*['"]github\.example\.com['"]/);
  });
});
