...
| M1.3 | **One-command install** — `npx -y @fyodoriv/minsky init` (or `curl ... \| sh`) on any git repo sets up everything; `minsky uninstall` cleanly removes it | 🟡 partial | **2026-06-10**: `@fyodoriv/minsky@0.1.0` published to npmjs.com (bare `minsky` permanently rejected by npm's similarity rule vs `minify` — pre-registered pivot executed). `bin/minsky init` exists, `./setup.sh` works for the clone-first path, tarball regression green (`MINSKY_RUN_INTEGRATION=1 pnpm vitest run test/integration/npx-init-tarball.test.ts`). Smoke passed same day: `npx -y @fyodoriv/minsky --version` → `0.1.0` from a clean npx cache. Remaining before ✅: integrated no-config `npx -y @fyodoriv/minsky` fresh-VM run (≤3 min to live iteration, the delivered `minsky-npx-install-and-run` acceptance) re-verified against the published package rather than the local tarball. |
...
| M1.13 | **Agents can self-heal Minsky** — when Minsky breaks, the running agent (or observer) diagnoses and fixes common failures without human intervention | 🟡 partial | 11 automated heals under `novel/observer/heals/src/`, all chaos-validated with MTTR < 5min (`novel/observer/heals/test/chaos/heal-catalogue-mttr.test.ts`). Runtime dispatcher (`scripts/heal-dispatch.mjs`) now wired at the pre-walk boundary of `bin/minsky-run.sh --loop`: stale-pid / corrupt-state-json / partial-config-write fire with production seams and write the live MTTR ledger at `.minsky/heal-events.jsonl`; missing-node-modules / stale-tsbuildinfo are dispatchable at `--boundary pre-spawn` (loop wire-in is a follow-up), and the 6 spawn-context heals (stuck-command, rate-limit pair, ollama-down, network-partition, brief-too-long) wire in at the spawn-failure boundary in a follow-up. Status stays 🟡 until ≥1 production heal-fire is observed — check with `node scripts/heal-mttr-report.mjs --window=30d --json` (source flips from `no-data` to `heal-events`). |
| M1.14 | **OpenHands as the canonical agent runtime** — `~/.minsky/config.json` accepts `cloud_agent: "openhands"`, the adapter ships, and `bin/minsky competitive --backend openhands` produces a comparable scorecard against `--backend claude` on the M1.10 corpus | 🔴 env-gap — live A/B blocked (2026-06-21) | Substrate (PR #777, #779, #780). **2026-05-24 (PR #782)**: `@minsky/agent-runtime-openhands` ships — Python shim over OpenHands SDK v1.19.1 + TS spawn-config builder; `cloud_agent: "openhands"` becomes the default; `pendingExternalDep` lifted ahead of schedule. **2026-06-21**: live A/B attempt hit two env-gap blockers: (1) `bin/minsky competitive` CLI was removed in Path-A phase-10 (2026-05-28); (2) `python3 -c "import openhands"` exits non-zero on the system Python — OpenHands SDK v1.7.0 is in `~/.minsky/openhands-venv/` but not on the system path. Pivot threshold crossed; documented in `competitors/openhands.md` § "M1.14 live A/B benchmark 2026-06-21"; follow-up `fix-openhands-sdk-env-import` filed as P0. Full plan: [`docs/plans/2026-05-22-path-c-openhands-reshape.md`](docs/plans/2026-05-22-path-c-openhands-reshape.md). <!-- exempt: scorecard IS the metric (same competitive-corpus surface as M1.10 exempt); the `bin/minsky competitive --backend openhands vs --backend claude` output is a comparison scorecard, not a single docs/METRICS.md row. Same exempt pattern as M1.10. Lift after the live A/B benchmark run (`openhands-vs-claude-m110-corpus-live-ab`) publishes the comparison output as part of the M1.10 corpus — the format is structurally identical to the existing M1.10 scorecard. --> |

**M1 summary** (2026-06-21) — done: 5 (M1.4, M1.6, M1.10, M1.12, M1.13); substrate ✅ + runtime/cross-machine gap: 2 (M1.8, M1.9); partial: 5 (M1.2, M1.3, M1.5, M1.7, M1.11); blocked: 2 (M1.1, M1.14 🔴 env-gap); not started: 0. The `bin/minsky milestone check` alignment gate reports 14/14 surfaces aligned — the table's status column reflects substantive feature completion which can lag the alignment gate.
...
2. Close M1.13 — observe ≥1 production heal-fire through the wired pre-walk dispatcher (`node scripts/heal-mttr-report.mjs --window=30d --json`), then wire the pre-spawn + spawn-failure boundaries.
...
8. Land `fix-openhands-sdk-env-import` (M1.14 closure) — restore the OpenHands vs Claude A/B comparison path (env-gap: system Python lacks `openhands` module; `bin/minsky competitive` CLI was removed in Path-A phase-10 2026-05-28). SDK v1.7.0 is in `~/.minsky/openhands-venv/`; fix wires the venv Python into the spawn path and adds a `--backend openhands` benchmark entry point. Attempt `openhands-vs-claude-m110-corpus-live-ab` 2026-06-21 hit both blockers; M1.14 now 🔴.
