...
| M1.10 | **Competitive benchmark — real, automated, weekly** — Minsky measures itself against ≥4 competitors (Devin, OpenHands, SWE-agent, Aider) on shared metrics; scorecard updates weekly | ❌ not started | `bin/minsky benchmark` runs internally for verdict distribution, but no `competitive-scorecard.json` exists yet. Tasks `self-metrics-competitive-benchmark` (P0) and `weekly-lmarena-style-benchmark` are open. |
