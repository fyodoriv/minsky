...
|---|---|---|---|
| M1.1 | **90% stability over 10h unattended runs** — `successful_iterations / total_iterations` across ≥5 consecutive 10h runs on ≥2 machines | 🟡 partial | **2026-06-21**: `node scripts/stability-report.mjs --window=7d --json` now reads `.minsky/session-ledger.jsonl` (primary, PR #1250 wire-in) instead of the empty experiment-store. Measurement: `[{"window":"7d","successful":6,"total":6,"ratio":1,"source":"session-ledger","generated_at":"2026-06-21T09:32:22.352Z"}]` — ratio=1.0 over 6 iterations (data accumulating since ledger was seeded 2026-06-21). Status is 🟡 partial because n=6 < 5 × 10h run threshold and only 1 machine reporting; full ✅ requires ≥5 consecutive 10h runs on ≥2 machines with ratio ≥ 0.90. Revisit after 24–48h of daemon runs. |
