...
| M1.13 | **Agents can self-heal Minsky** — when Minsky breaks, the running agent (or observer) diagnoses and fixes common failures without human intervention | ✅ done | 11 automated heals under `novel/observer/heals/src/`, all chaos-validated with MTTR < 5min (`novel/observer/heals/test/chaos/heal-catalogue-mttr.test.ts`). Runtime dispatcher (`scripts/heal-dispatch.mjs`) wired at the pre-walk boundary of `bin/minsky-run.sh --loop`. **2026-06-15**: first production heal fire observed — `.minsky/heal-events.jsonl` entry `ts=2026-06-15T12:18:04.836Z class=stale-pid outcome=healed` (2 total fires in 30d window; `node scripts/heal-mttr-report.mjs --window=30d` source=`heal-events`, successful=2/2, p50=12ms). Pre-walk dispatcher confirmed functional in production. |
...
2. ~~Close M1.13~~ — **done 2026-06-21**: 2 production heal fires observed (stale-pid, 2026-06-15 + 2026-06-17); M1.13 flipped to ✅. Pre-spawn + spawn-failure boundary wire-in is a P2 follow-up.
