...
class TestResolveGhHostParity:
    ...
    def test_https_github_example_com_remote(self) -> None:
        # TS: "https github.example.com remote → github.example.com"
        assert resolve_gh_host(
            env_gh_host=None,
            git_remote_url="https://github.example.com/team/repo.git",
        ) == ResolveGhHostResult(host="github.example.com", source="git-remote")

    def test_ssh_github_example_com_remote(self) -> None:
        # TS: "ssh github.example.com remote → github.example.com"
        assert resolve_gh_host(
            env_gh_host=None,
            git_remote_url="git@github.example.com:team/repo.git",
        ) == ResolveGhHostResult(host="github.example.com", source="git-remote")
    ...
    def test_env_github_com_overrides_example_remote(self) -> None:
...
class TestCli:
    ...
    def test_cli_remote_override(self, tmp_path: Path) -> None:
        ...
        result = self._run_cli(
            "--host-root", str(tmp_path),
            "--git-remote-url", "git@github.example.com:team/repo.git",
        )
        assert result.returncode == 0, result.stderr
        assert result.stdout.splitlines() == ["github.example.com", "git-remote"]
