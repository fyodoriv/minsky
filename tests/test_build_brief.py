...
def test_render_brief_includes_task_id() -> None:
    ...
    brief = build_brief.render_brief(task, "fyodoriv/minsky", "feat/aifn-840-slash-command-labels")
    assert "# Task: aifn-840-slash-command-labels" in brief
...
def test_render_brief_includes_host_repo_and_branch() -> None:
    ...
    assert "Branch: feat/aifn-840-slash-command-labels" in brief
...
def test_render_brief_includes_all_5_rule_9_fields() -> None:
    ...
    assert "yarn vitest run plugins/iep-ai-native" in brief
...
def test_build_brief_concatenates_task_block_and_overlay_with_separator() -> None:
    ...
    assert "\n---\n" in brief
    # Task block first.
    assert brief.index("# Task: aifn-840-slash-command-labels") < brief.index("\n---\n")
    # Overlay after.
...
def test_cli_prints_brief_to_stdout(tmp_path: Path) -> None:
    ...
    result = _run_cli(["aifn-840-slash-command-labels", str(tmp_path)], cwd=tmp_path)
    assert result.returncode == 0
    assert "# Task: aifn-840-slash-command-labels" in result.stdout
