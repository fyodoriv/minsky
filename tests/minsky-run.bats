...
#   self-heals after a salvage-merge (2026-05-16 oncall-hub-plugin
#   regression).
#
  ...
  "pr list")
    ...
    for ((i=1; i<=$#; i++)); do
      if [[ "${!i}" == "--head" ]]; then
        ...
        # branch's open PR).
        echo "https://github.intuit.com/team/backstop-recovery/pull/77"
        exit 0
      fi
    done
    echo "" ; exit 0 ;;
...
EOF
  ...
  grep -q '"pr_url":"https://github.intuit.com/team/backstop-recovery/pull/77"' "$record_file"
