...
/**
 * CLI: reads the three required unit files from
 * `distribution/systemd/` and runs the pure check.
 *
 * Exit codes:
 *   0 — pass
 *   1 — missing directive(s) — fail with a per-pair report
 *   2 — I/O error reading a unit file (rule-#6 let-it-crash)
 *
