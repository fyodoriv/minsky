...
export function readLedger(ledgerPath) {
  ...
  for (const line of raw.split("\n")) {
    ...
    try {
      ...
      // Malformed line — skip. The reporter must not crash on bad data.
    }
  }
  return events;
}
...
export function computeWindowStats(args) {
  ...
  const successful = inWindow.filter((e) => e.outcome === "healed");
  const durations = successful.map((e) => e.duration_ms);
  return {
    window,
  ...
  if (jsonMode) {
    ...
    for (const r of rows) {
      if (r.source === "no-data") {
        ...
        console.log(
          `${r.window}: ${r.successful}/${r.attempted} healed; p50=${p50} p95=${p95}`,
        );
      }
    }
  }
