...
/**
 * Minimal CSS — system font stack, dark background, responsive grid of
 * metric cards, muted styling for the `(stub)` not-wired-yet state.
 * Inlined to keep the dashboard a single SSR response (no extra fetch
 * round-trip; survives the operator opening the URL while offline).
 *
 * Anchor: Card & Mackinlay 1999 (glanceable display — label + value +
 * unit in the same focal point); Wilkie 2018 (RED method service-level
 * lens — values are the focal element, formulas the provenance trail
 * in muted code-style).
 */
