...
### `POST /control` requires a per-run token (slices 1–2 shipped)
...
Slice 2 (this PR) wires `validateControlAuth` into the `POST /control` route via a new `createServer({ controlToken })` arg. The auth check runs before body parse — bad/missing `X-Minsky-Token` header returns `401 Unauthorized` regardless of body shape (fail-fast), and the `setPaused` Strategy is never called on the unauthenticated branch (verified by call-counter assertions). When `controlToken` is `undefined` (the v0 default + existing test wiring), no validation runs — preserving the backward-compat path until slice 3 lands. Slice 3 wires `resolveControlToken` + `controlTokenStartupHint` into `src/start.ts` so production boots with a real token; at that point the `controlToken=undefined` fail-open default closes. Until slice 3, the bind-default (127.0.0.1) keeps the surface unreachable from the LAN, so the per-run-secret gate is defense-in-depth. Paired tests live in `test/control-auth.test.ts` (15 cases) and `test/server.test.ts` (11 new cases — missing header / wrong token / case-insensitive header lookup / `controlToken=undefined` preserves v0 behavior / constant-time wiring / `setPaused` never called on 401).
