...
- [ ] Fix the slash command labels AIFN-840
...
describe("parseTasksMd", () => {
  test("parses two tasks under P0", () => {
    ...
    expect(tasks[0]?.id).toBe("aifn-840-slash-command-labels");
  ...
  test("captures multi-line continuation lines under **Details**:", () => {
    // Regression — discovered 2026-05-16 on the oncall-hub-plugin run:
...
`;
    ...
    expect(t?.anchor).toBe("2026-05-16 oncall-hub-plugin run");
...
describe("findTask — exact ID match", () => {
  test("returns the task when ID matches exactly", () => {
    const result = findTask(sampleTasksMd, "aifn-840-slash-command-labels");
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.task.id).toBe("aifn-840-slash-command-labels");
  });

  test("ID match is case-sensitive (kebab-IDs are lower-case by convention)", () => {
    // Exact-ID match is case-sensitive. The mixed-case query "AIFN-840-..."
    // is NOT a substring of the title (which is "...labels AIFN-840"), and
...
describe("findTask — title substring (ticket-key matching)", () => {
  test("AIFN-840 matches the title", () => {
    const result = findTask(sampleTasksMd, "AIFN-840");
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.task.id).toBe("aifn-840-slash-command-labels");
  });
...
describe("findTask — not found", () => {
  test("returns ok:false with available IDs when no match", () => {
    ...
    expect(result.ok).toBe(false);
    if (result.ok) return;
    expect(result.availableIds).toContain("aifn-840-slash-command-labels");
...
describe("pickHostTask", () => {
  test("returns the first rule-#9-compliant P0 task", () => {
    ...
    expect(task?.id).toBe("aifn-840-slash-command-labels");
