...
// other authenticated host (`github.intuit.com`) which produces:
//
//     HTTP 401: Must authenticate to access this API. (https://github.intuit.com/api/graphql)
...
describe("resolveGhHost — pure", () => {
  ...
  test("https github.intuit.com remote → github.intuit.com", () => {
    expect(
      resolveGhHost({
        envGhHost: undefined,
        gitRemoteUrl: "https://github.intuit.com/team/repo.git",
      }),
    ).toEqual({ host: "github.intuit.com", source: "git-remote" });
  });

  test("ssh github.intuit.com remote → github.intuit.com", () => {
    expect(
      resolveGhHost({
        envGhHost: undefined,
