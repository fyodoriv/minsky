...
3. **`empty-queue`** — `pickHostTask` returns null (no rule-#9-compliant `P0`/`P1` task left, OR every remaining candidate carries a non-empty `**Blocked**:` reason). Healthy stop. The picker filters blocked tasks via the `isNotBlocked` predicate so an iteration is never spent on a task whose external-dep / approval gate has not yet resolved; the `**Blocked by**: <id>` task-dependency form is handled at a separate dependency-graph layer.
...
minsky-run proj-840-slash-command-labels --host ~/apps/example-capabilities-3
...
minsky-run proj-840-slash-command-labels --host ~/apps/example-capabilities-3 --live
```
...
minsky-run --host ~/apps/example-capabilities-3

# The aggregate is opt-out — every flag can be disabled individually:
minsky-run --host ~/apps/example-capabilities-3 --no-live        # dry-run loop
minsky-run --host ~/apps/example-capabilities-3 --once           # single iteration
minsky-run --host ~/apps/example-capabilities-3 --no-cto-audit   # ship-only, no audit
minsky-run --host ~/apps/example-capabilities-3 --no-seed-on-empty  # exit on empty queue
```
...
```bash
cd ~/apps/example-capabilities-3 && minsky-run     # single-host mode
...
2. **Indented-only** (legacy fixture form; used by `test/integration/proj-840-shape.test.ts`):
...
- `proj-840-shape.test.ts` (20 integration cases) — end-to-end bootstrap → minsky-run smoke; autonomous-default aggregate; `--hosts-dir` walk; `--host` + `--hosts-dir` mutual exclusion
