...
- **`MINSKY_CTO_AUDIT_ENABLE=1|true`** — opt in to post-task CTO audits (sub-step (d/e/f) of `post-task-cto-audit`). When set, the CLI constructs the `CtoAuditSeam` `runDaemon` expects: the daemon's existing `spawnStrategy` is reused as `CtoAuditSpawn` (structurally compatible per task spec sub-step (a)); the lock is file-backed at `<MINSKY_HOME>/.minsky/cto-audit-lock/<id>` so the cap-1-per-task contract (sub-step f) survives daemon restart; signals are collected from `git log` + `gh issue/pr list` with rule-#7 graceful-degrade (offline / rate-limit yields zero counts, not a crash). Default OFF — the audit's prompt-engineering surface ships behind an explicit flag (rule #9 pivot threshold #1: don't fire >5 audits/day on first rollout). The audit's per-iteration gate (`shouldRunCtoAudit`) still respects `MINSKY_CTO_AUDIT=off` for ad-hoc skips even when the seam is wired.
- **`MINSKY_CHANGELOG_ENABLE=1|true`** — opt in to the daily changelog runner (acceptance criterion (3) of `daily-changelog-for-humans`). When set, the CLI constructs the `ChangelogSeam` `runDaemon` expects: the daemon's existing `spawnStrategy` is reused as `ChangelogSpawn` (structurally compatible per task spec sub-step (a)); `readChangelog` is file-backed via `createFileBackedChangelogReader(<MINSKY_HOME>/CHANGELOG.md)` with ENOENT graceful-degrade (a fresh checkout pre-genesis still fires — the runner authors the genesis entry). Default OFF — same rule #9 pivot threshold rationale as CTO-audit (>1 spawn/day on first rollout would breach the per-day idempotency contract). The runner's per-iteration gate (`shouldRunChangelog`) still respects `MINSKY_CHANGELOG=off` for ad-hoc skips even when the seam is wired; idempotency comes from the `## YYYY-MM-DD` H2 header in CHANGELOG.md (one source of truth, rule #2 — the section header IS the "this happened" record).
