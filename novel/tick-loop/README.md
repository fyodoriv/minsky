...
- **Mutual exclusion via uniquely-keyed locks**: Lamport, "A New Solution of Dijkstra's Concurrent Programming Problem", CACM 1974. `deriveClaimKey(repo, task)` derives one shared O_EXCL lock path per `(repo, task)` so two processes on the same repo serialise to one task winner, while different tasks (or different repos) never cross-block. **Conformance: full** — the OS `O_EXCL` create IS the bakery-ticket; this module supplies the per-(repo,task) ticket name. The pre-spawn claim gate (`scripts/orchestrate.mjs pre-spawn-claim`) wraps this lock + the pure `decidePreSpawnClaim` decision so a parallel-worker fanout (`bin/minsky-run.sh --workers-total N`) hands each worker a different task; the claim-aware picker skip (`scripts/pick_task.py --claimed-task-ids`) is the soft pre-check before the OS lock.
- **Liveness via stale-lock eviction**: Lamport 1974 (the bakery's liveness property — a crashed ticket-holder must not block the queue forever). `decideStaleClaimEviction(claims, ttlMs, nowMs)` is the pure partition that, every conductor tick, evicts any claim whose age exceeds the iteration TTL (its owning run died mid-iteration, leaving an orphaned O_EXCL lock) and keeps every live claim — returning the wedged task to the pickable pool. A backwards clock (a future-stamped claim) is treated as fresh, never evicted, so a clock fault degrades to the safe side (rule #6). The I/O (readdir + stat the lock dir for mtimes, unlink the evicted paths) lives at the conductor edge in `scripts/orchestrate.mjs`; this module is the decision. **Conformance: full.** Slice (d) of `daemon-parallel-worktree-launch`.
...
| 21 | A worker crashes mid-iteration, leaving an orphaned `(repo, task)` O_EXCL claim lock that would wedge the task forever (no sibling could ever win it) | crashed-holder-deadlock | `graceful-degrade` — `decideStaleClaimEviction` evicts every claim older than the iteration TTL each conductor tick, returning the wedged task to the pool; a live worker's claim is never preempted; a backwards clock degrades to keeping claims | `novel/tick-loop/src/worker-config.test.ts` (decideStaleClaimEviction TTL + clock-skew assertions) + `scripts/chaos-multitenant.test.mjs` (zero staleClaimMisEvictions assertions) |
| 22 | The daemon's N-worker fanout (one walker, N workers) collides on a per-worker namespace, OR two workers both spawn on the same top-priority task (the picker's claim-aware skip raced) | concurrency-collision | `graceful-degrade` — each worker's per-worker run-id keys a disjoint namespace (zero `fanoutCollisions`); the pre-spawn claim gate (`decidePreSpawnClaim` + the O_EXCL `acquireTaskClaim`) refuses the second worker, which re-picks | `scripts/chaos-multitenant.test.mjs` (countFanoutCollisions assertions) + `scripts/orchestrate.test.mjs` (decidePreSpawnClaim + acquireTaskClaim assertions) + `tests/minsky-run.bats` (fanout-worker-skips-claimed-task) |
