...
|---|---|---|
| Spoofing | TASKS.md modified between claim and spawn; daemon acts on a different task | Claim writes a lock entry before spawn; `git diff` post-spawn detects unexpected mutations |
| Tampering | Iteration logs or `CHANGELOG.md` modified to falsify measurement history | Files are committed; `supply-chain-hardening-lockfile-sbom-slsa` P0 adds SLSA provenance |
| Repudiation | Spawned Claude Code stdout capture is limited to 4 KB tail; child actions may not appear | 4 KB limit is intentional (rule #7); git history of affected files is the authoritative record |
| Information Disclosure | `~/.claude/projects/<cwd>/<session>.jsonl` contains raw conversation content | File is read-only; never transmitted; `otel-no-pii-in-spans-lint` P0 ensures OTEL spans carry no PII |
| Denial of Service | Runaway spawned Claude process; disk exhaustion from unbounded log growth | `BudgetGuard` circuit-breaks at ≥85 % token consumption; log files bounded by OS quota |
| Elevation of Privilege | Spawned Claude Code has full filesystem access; no syscall restriction yet applied | `supervisor-sandbox-syscall-restriction` P0 adds `ProtectSystem=strict` + macOS App Sandbox |
