...
## Daily changelog (operator-readable historical narrative)

After every supervisor iteration, the daemon checks "is today's date already in `CHANGELOG.md`?" — if not, fires `runChangelog({signals, lockPath})` (in `src/changelog-runner.ts`, follow-up to #180's substrate) which composes a date-gate (skip if today's section exists) + a file-backed lock (one author per host) + the pure builder from `scripts/generate-changelog-entry.mjs` (#179). Disable via `MINSKY_CHANGELOG=off`. Mirrors the `runCtoAudit` pattern — both are post-iteration audit steps; CTO audit fires per shipped task, changelog fires once per day. Composes with the dashboard live render (P1) and the marketing timeline (P3).
...
The CLI in `bin/tick-loop.mjs` is the I/O boundary for four optional channels — each is opt-in via env var and graceful-degrades when unset (rule #7):
