...
describe("bin/minsky.mjs drift — dist-missing message", () => {
  // The dist-existence check in `bin/minsky.mjs` deliberately INLINES a
  // copy of the message string (rather than importing
  // `formatDistMissingMessage` from `dist/`) — the whole point of the
  // check is that `dist/` may be missing, so the check itself cannot
  // depend on it. The inline copy and this canonical formatter therefore
  // live in two places. This test pins them: any wording change in one
  // must be reflected in the other or CI fails.
  //
  // Comparison technique: split the canonical output around a sentinel
  // path placeholder, normalize the on-disk source's escaped backticks
  // (`\\\`` → `\``) so the JS template-literal source matches its
  // runtime expansion, then assert both structural halves appear in the
  // bin file.
