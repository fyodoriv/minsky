...
export function buildDaemonBrief(args: {
  ...
  return [
    ...
    "",
    "Run `pnpm pre-pr-lint` before `gh pr create` (same gate as CI and `lefthook pre-push`; source: `scripts/run-pre-pr-lint-stack.mjs`; rule #10). Up to 3 fix attempts on failure. Still red after 3 → output `noop, exiting — pre-pr-lint-failures: <step name>` and DO NOT open a PR.",
    "",
