...
export function buildDaemonBrief(args: {
  ...
  return [
    ...
    "",
    "`pnpm pre-pr-lint` before `gh pr create`. Green → proceed; red → fix (stderr names step), retry ≤3×; still red → `noop, exiting — pre-pr-lint-failures: <step name>`, no PR.",
    "",
    ...
    "```",
    "",
    "Forbidden: `**Predicted:**` or `**Match:**` with colon INSIDE bold, or capitalized `Yes` — use `**Predicted**: …`, `**Match**: yes`/`no`/`partial`.",
    "",
    "## PR security-review template",
    "",
    "`scripts/check-pr-security-review.mjs` (vision.md § 13): ONE of these is required in every PR body:",
    "- Security surface touched: add `## Security & privacy` heading with analysis",
    "- No security surface: `<!-- security: not-applicable — <reason ≥3 chars> -->`",
    "",
