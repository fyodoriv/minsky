...
export function buildDaemonBrief(args: {
  ...
  return [
    ...
    "",
    "Every PR body needs one or CI lint `check-pr-security-review.mjs` fails. **Option A** — security surface (auth, secrets, PII, sandbox, supply chain): add `## Security & privacy` heading + `<threat + mitigation or 'no new attack surface; vision.md § 13 minimum bar reviewed'>`. **Option B** — no security surface: `<!-- security: not-applicable — <reason ≥3 chars> -->` (`--` also accepted). When in doubt, Option A.",
    "",
