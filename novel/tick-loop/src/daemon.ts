...
export function buildDaemonBrief(args: {
  ...
  return [
    ...
    "",
    "Body-only checks (`pr-self-grade`, `pr-security-review`): write the draft body to a file (e.g. `pr-body.md`), then run `pnpm pre-pr-lint -- --body=pr-body.md` — the flag appends both checks to the same gate. One command, one retry budget. Then `gh pr create -F pr-body.md`.",
    "",
