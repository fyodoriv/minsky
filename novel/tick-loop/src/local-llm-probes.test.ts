...
describe("buildProductionProbes + detectLocalLlmStack + planLocalLlmBootstrap — selectively-missing integration", () => {
  it("fresh machine (nothing present) → full 6-step plan", async () => {
    const probes = buildProductionProbes({
  ...
  it("pipx + mlx + huggingface-cli absent, aider present, model present, server stopped → [install-pipx, install-mlx-lm, install-huggingface-cli, start-mlx-server]", async () => {
    const probes = buildProductionProbes({
