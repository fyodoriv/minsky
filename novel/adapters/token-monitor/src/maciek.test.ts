...
describe("MaciekTokenMonitor", () => {
  it("single-block fixture: 3000 tokens used → 85_000 remaining for max5", async () => {
    ...
    expect(s.windowSizeTokens).toBe(88_000);
  ...
  it("dedup: same (message.id, requestId) across two files counts once", async () => {
    ...
    const s = await tm.snapshot();
    // Each file contributes 3000 tokens; without dedup → 6000 used → 82_000 remaining.
    // With dedup → 3000 used → 85_000 remaining.
    expect(s.tokensRemainingInWindow).toBe(85_000);
  });
