...
| 3 | Model returns malformed JSON inside `structured()` | upstream-malformed | `loud-crash-supervisor-restart` — `JSON.parse` throws synchronously; caller decides whether to retry or pivot to `runABTest` | covered by `novel/adapters/prompt-optimizer/src/structured.test.ts` "throws synchronously (rejects) when the model returns non-JSON text" assertion |
