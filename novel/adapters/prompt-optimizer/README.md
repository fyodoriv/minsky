...
| 3 | Model returns malformed JSON inside `structured()` | upstream-malformed | `loud-crash-supervisor-restart` — `JSON.parse` throws synchronously; caller decides whether to retry or pivot to `runABTest` | (deferred — covered when `mape-k-plan-execute-phases` ships) |
