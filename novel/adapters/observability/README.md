...
| 4 | TRACEPARENT propagation broken across subagent boundary | run a subprocess that doesn't honor `OTEL_PROPAGATORS` (upstream-malformed) | `circuit-break-and-notify` (future) | (deferred — covered when `traceparent-subagent-propagation-test` ships) |
