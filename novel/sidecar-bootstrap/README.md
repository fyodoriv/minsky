...
| 5 | Global git ignore file is read-only | filesystem-permission | `graceful-degrade` — bootstrap retries the append against `<host>/.git/info/exclude` (per-clone), prints a `⚠ global ignore unwritable; fell back to per-clone exclude` line, exits 0. Non-EACCES errors (typo, missing parent dir, ENOENT-after-mkdir) are NOT retried — the operator's setup needs manual attention. | covered by `ignore-write.test.ts` (`wrote-per-clone when global EACCES + clone OK` + `error when both global EACCES AND clone EACCES` + `fallback attempts the clone path only after global EACCES` + `error when global returns 'other'` + `classifyWriteError maps EACCES/EPERM/EROFS to 'eacces'` test cases) |
