...
| 5 | Global git ignore file is read-only | filesystem-permission | `graceful-degrade` — bootstrap continues but the operator must add `.minsky/` to a writable ignore manually; v0 throws here, v1 falls back to per-clone-exclude per the umbrella story's chaos table row #13 | (deferred — covered when `cross-repo-runner-v1-live-spawn` ships and this fallback is exercised) |
