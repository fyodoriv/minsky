...
- **Paper**: Xia, Deng, Dunn, Zhang, "Agentless: Demystifying LLM-based Software Engineering Agents", *arXiv* 2407.01489, 2024 (accepted to *ICSE* 2026; the project is the basis of OpenAI's SWE-bench Verified scaffold reported in the GPT-4o/o1 system cards).
...
The paper's headline (SWE-bench Lite, GPT-4o driver): **27.33% resolve rate at ~$0.34 average cost per issue** — at the time, the best-performing AND lowest-cost open-source entry on the Lite leaderboard. Later Agentless configurations and driver models pushed higher (the OpenAutoCoder harness is the scaffold behind several SWE-bench Verified submissions in the 30-40%+ range depending on the driver). These are **Lite/Verified resolve rates of the pipeline**, not a Minsky metric — and that is exactly the point of running it head-to-head: the number Minsky-via-`<agent>` posts on the same corpus is the comparison, and a fixed pipeline beating an agent loop on a task-class is the falsification signal.
...
| --- | --- | --- |
| `swe-bench-verified-resolve-rate` (and cost) | `local-harness` (`agentless-swebench-lite`) | Xia, Deng, Dunn, Zhang, "Agentless: Demystifying LLM-based Software Engineering Agents", arXiv 2407.01489, 2024 (SWE-bench Lite 27.33% at ~$0.34/issue with GPT-4o; OpenAutoCoder/Agentless harness scripts). |
...
2026-06-02 — deepened from stub with `## Should we adopt the Agentless pipeline instead?` + `## Five pivot questions` (Five Pivot Questions framework) per task `competitor-deep-research-tier-s-2026-05`. Verdict: REFERENCE / thesis falsifier; ADD to the benchmark corpus as a `local-harness` row (`agentless`, harness `agentless-swebench-lite`) — required and the one corpus-landing result of the four Tier-S stubs; optionally ADD an Agentless-style fixed pipeline as a cheap routing target for test-anchored bug fixes (gated on a pre-registered routing experiment); absorb the synthesised-reproduction-test validation oracle. No vision change — the Pivot was tested: Agentless is measurably cheaper/lower-variance on bug-fix-shaped tasks (not noise, so the corpus seat is earned) but is single-shot and bug-fix-shaped, so it sharpens *where* Minsky claims value rather than dissolving the moat (negative-on-moat / positive-on-corpus finding logged inline per this task's central-questions routing).

Earlier reviews: 2026-05-22 (STUB — deep research pending; flagged as the highest-priority of the five Tier-S stubs).
