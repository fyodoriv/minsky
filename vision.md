...
| 26 | `@minsky/budget-guard` (`BudgetGuard` watchdog + `decide()` pure decision function) | Watchdog (periodic-deadline check loop, hardware / OS literature) + error-budget burn-rate alerting | Beyer et al., *Site Reliability Engineering*, Ch. 3, 2016 (error budgets); failure-mode response labels from rule #7 | partial | v0 ships pure decision logic only. `.minsky/budget.flag`, `localhost:9876` HTTP API, and Maciek-impl Strategy ship in follow-up sub-tasks (`budget-guard-flag-file`, `budget-guard-http-api`, `budget-guard-maciek-impl`). Path deviation: original task brief specified `/var/run/minsky/budget.flag` which would need root; v0 follow-up uses `${MINSKY_HOME}/.minsky/budget.flag` instead — root-required paths out of scope for solo-dev tier. |
