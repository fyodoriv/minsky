...
| 35 | `novel/spec-monitor/` (`SKILL.md` + `test/judgement-only/` + `test/deterministic-overlap/` + `spec-advisories/`) | Runtime verification split into deterministic-monitor + advisory-judgement layers; advisory-only Claude Skill | Havelund & Goldberg, "Verify Your Runs", *VSTTE* 2008 (runtime verification); Hunt & Thomas, *The Pragmatic Programmer*, 1999, Tip 32 ("crash early"); rule #10 (vision.md § 10 — deterministic enforcement, ratchet rule: when a deterministic linter is added the matching Skill check is removed); rule #8 (pattern conformance — the Skill advises on conformance-level mismatches the linter cannot read). | partial | Advisory-only Skill, never gates CI — the deterministic linters at `scripts/check-rule-{1,2,3,4,5,6,7}-*.mjs`, `scripts/check-pattern-index.mjs` and `scripts/check-pr-self-grade.mjs` carry the load-bearing share. The Skill addresses ≤5 residual judgement-heavy concerns (A1 hypothesis vagueness, A2 pivot equals success, A3 non-primary anchor, A4 measurement output unchecked, A5 conformance level mismatched to source); growing past 5 requires either retiring one or shipping a deterministic linter (rule-#10 ratchet). **Deviation from Havelund-Goldberg 2008:** classical runtime verification produces deterministic verdicts; this Skill produces advisory verdicts only. The deterministic share of runtime verification lives in the linters above; this row covers the residual *judgement* share that rule #10 explicitly carves out as "what cannot be deterministically checked is not a constitutional rule" — kept as an advisory companion, not an enforcement mechanism. |
